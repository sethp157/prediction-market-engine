"""One-off EXP-0002 analysis. prepare exposes training/validation only; evaluate exposes tests."""
import argparse
import collections
import csv
...
GRID = [(c,s,w) for c in [-2,-1,0,1,2] for s in [1.5,2,3,4,5] for w in [0,.25,.5,.75,1]]
...
def split(day):
    ...
    return "test_b"
def weather_probabilities(intervals, forecast, c, s):
    return np.array([ndtr((hi+.5-forecast-c)/s)-ndtr((lo-.5-forecast-c)/s) for lo,hi in intervals])
def predict(e, config):
    c,s,w=config
    r=np.asarray(e["r"],float)
    if w==0: return r.copy()
    intervals=[(-math.inf if lo is None else lo,math.inf if hi is None else hi) for lo,hi in e["intervals"]]
    q=weather_probabilities(intervals,e["mos_forecast"],c,s)
    if np.any(q<0) or not np.isclose(q.sum(),1,atol=1e-12): raise RuntimeError("Invalid weather probability vector")
    return (1-w)*r+w*q
def score(p, y): return float(np.mean((np.asarray(p) - np.asarray(y)) ** 2))

def mos_runtime(day):
    return dt.datetime.combine(day-dt.timedelta(days=1),dt.time(6),dt.timezone.utc)

def mos_filename(day):
    return "mos-KMIA-GFS-"+mos_runtime(day).strftime("%Y%m%d%H")+".json"

def mos_utc(s):
    # IEM's explicitly named *_utc fields can omit the Z suffix.
    if not isinstance(s,str) or not s: raise ValueError("Missing MOS UTC timestamp")
    parsed=dt.datetime.fromisoformat(s.replace("Z","+00:00"))
    if parsed.tzinfo is None: parsed=parsed.replace(tzinfo=dt.timezone.utc)
    if parsed.utcoffset()!=dt.timedelta(0): raise ValueError("MOS UTC field not UTC")
    return parsed

def read_mos(day,decision_ts):
    path=ROOT/"raw"/mos_filename(day)
    if not path.exists(): return {},["mos_request_failed"]
    d=json.loads(path.read_bytes()); rows=d.get("data")
    if not isinstance(rows,list) or not rows: return {},["mos_missing_rows"]
    runtime=mos_runtime(day); required=runtime+dt.timedelta(hours=42)
    assumed=runtime+dt.timedelta(hours=6)
    if assumed.timestamp()>=decision_ts: return {},["mos_assumed_publication_after_cutoff"]
    selected=[]
    for r in rows:
        try:
            if r["station"]!="KMIA" or r["model"]!="GFS" or mos_utc(r["runtime_utc"])!=runtime:
                return {},["mos_station_model_runtime_mismatch"]
            valid=mos_utc(r["ftime_utc"])
            if valid==required: selected.append(r)
        except (KeyError,ValueError,TypeError): return {},["mos_malformed_metadata"]
    if len(selected)!=1: return {},["mos_missing_or_duplicate_horizon"]
    x=selected[0].get("n_x")
    if isinstance(x,bool) or not isinstance(x,(float,int)) or not math.isfinite(x) or not -100<=x<=150:
        return {},["mos_missing_or_invalid_high"]
    return {"mos_forecast":float(x),"mos_runtime_utc":runtime.isoformat(),
            "mos_valid_utc":required.isoformat(),"mos_assumed_available_utc":assumed.isoformat()},[]

def verify_inputs():
    entries={}; allowed_urls={BASE+"/historical/cutoff"}
    ...
    rawfiles=[p for p in (ROOT/"raw").rglob("*") if p.is_file()]
    if set(entries)!=set(p.relative_to(ROOT/"raw").as_posix() for p in rawfiles): raise RuntimeError("Manifest/file inventory mismatch")
    for path in rawfiles:
        name=path.relative_to(ROOT/"raw").as_posix()
        if name not in entries: raise RuntimeError("Raw file without provenance: "+name)
        if hashlib.sha256(path.read_bytes()).hexdigest()!=entries[name]["sha256"] or path.stat().st_size!=entries[name]["bytes"]:
            raise RuntimeError("Raw hash mismatch: "+path.name)
        began=iso_ts(entries[name]["request_started_at_utc"])
        ended=iso_ts(entries[name]["response_received_at_utc"])
        if ended<began: raise RuntimeError("Invalid request-time provenance")
        if not name.startswith("errors/") and entries[name]["status_code"]!=200: raise RuntimeError("Invalid successful response status")
    errors={k:v for k,v in entries.items() if k.startswith("errors/")}
    entries={k:v for k,v in entries.items() if not k.startswith("errors/")}
    allmarkets=[]; previous_cursor=""
    pages=sorted((ROOT/"raw").glob("markets-*.json"))
    if not pages: raise RuntimeError("No metadata pages")
    for page,path in enumerate(pages):
        ...
        url=BASE+"/historical/markets?"+urllib.parse.urlencode(params); allowed_urls.add(url)
        if entries[path.name]["url"]!=url:
    ...
    if len(set(m["ticker"] for m in selected))!=len(selected): raise RuntimeError("Duplicate study metadata")
    ...
    expected_mos=set(); expected_mos_map=[]
    for offset in range((END-START).days+1):
        day=START+dt.timedelta(days=offset); name=mos_filename(day); expected_mos.add(name)
        runtime=mos_runtime(day)
        expected_mos_map.append({"target_date":day.isoformat(),"station":"KMIA","model":"GFS",
            "runtime_utc":runtime.isoformat(),"required_ftime_utc":(runtime+dt.timedelta(hours=42)).isoformat(),
            "assumed_available_utc":(runtime+dt.timedelta(hours=6)).isoformat(),"raw_file":name,"response_ok":name in entries})
        params={"station":"KMIA","runtime":mos_runtime(day).strftime("%Y-%m-%dT%H:%MZ"),"model":"GFS"}
        url="https://mesonet.agron.iastate.edu/api/1/mos.json?"+urllib.parse.urlencode(params)
        allowed_urls.add(url)
        if name not in entries: continue
        if entries[name]["url"]!=url: raise RuntimeError("Unexpected MOS URL: "+name)
    if set(p.name for p in (ROOT/"raw").glob("mos-*.json"))-expected_mos: raise RuntimeError("Unexpected MOS file")
    if load("mos-study.json")!=expected_mos_map: raise RuntimeError("MOS derived request map mismatch")
    allowed=set(p.name for p in pages)|expected_candles|expected_mos|{"cutoff.json"}
    if set(entries)-allowed: raise RuntimeError("Unrecognized source file")
    if any(e["url"] not in allowed_urls for e in errors.values()): raise RuntimeError("Unexpected error-response URL")
    return {"raw_files_verified":len(rawfiles),"metadata_pages_verified":len(pages),
            "mos_files_verified":len(set(entries)&expected_mos),"error_responses_verified":len(errors),
            "study_metadata_matches_sources":True,"mos_request_map_verified":True}
...
def interval(m):
    if m["strike_type"]=="less": return [None,int(m["cap_strike"])-1]
    if m["strike_type"]=="greater": return [int(m["floor_strike"])+1,None]
    return [int(m["floor_strike"]),int(m["cap_strike"])]

def prepare():
    if (ROOT/"fit-lock.json").exists() or (ROOT/"results.json").exists():
        raise RuntimeError("Existing fit or exposure: do not overwrite the research record")
    ...
    for offset in range((END-START).days+1):
        ...
        g=f"KXHIGHMIA-{day.year%100:02d}{mon}{day.day:02d}"
        ...
        reasons=[]
        mos,mos_reasons=read_mos(day,t)
        row["mos_valid"]=not bool(mos_reasons)
        reasons.extend(mos_reasons)
        if not markets: reasons.append("no_event_metadata")
        elif not partition_ok(markets): reasons.append("invalid_bin_partition")
        quotes=[]
        for m in markets:
            rules=m.get("rules_primary")
            if not isinstance(rules,str): reasons.append("rules_missing_or_malformed")
            elif "miami" not in rules.lower() or "national weather service" not in rules.lower(): reasons.append("rules_mismatch")
            try:
        ...
        row["market_eligible_without_mos"]=not any(not reason.startswith("mos_") for reason in reasons)
        if reasons:
            ...
            accepted.append(dict(row,quotes=quotes,y=labels,r=r,b=b,
                                 intervals=[interval(m) for m in markets],**mos))
        rows.append(row)
    ...
    train_grid=[{"config":list(cfg),"brier":float(np.mean([score(predict(e,cfg),e["y"]) for e in train]))} for cfg in GRID]
    best=min(x["brier"] for x in train_grid)
    candidates=[x for x in train_grid if x["brier"]<=best+1e-12]
    config=min(candidates,key=lambda x:(x["config"][2],abs(x["config"][0]),-x["config"][1],x["config"][0]))["config"]
    counts={}
    for s in ["train","validation","test_a","test_b"]:
        ...
        counts[s]={"expected":len(rr),"metadata_present":sum(r["returned_bins"]>0 for r in rr),
                   "mos_valid":sum(r["mos_valid"] for r in rr),
                   "market_eligible_without_mos":sum(r["market_eligible_without_mos"] for r in rr),
    ...
    pre={"created_utc":dt.datetime.now(dt.timezone.utc).isoformat(),"config":config,"config_order":["bias_F","standard_deviation_F","weather_weight"],"training_grid":train_grid,"provenance_checks":provenance,
         ...
         "dependency_sha256":{name:hashlib.sha256((ROOT/name).read_bytes()).hexdigest() for name in ["coverage.json","collect.py","protocol.md","clarifications.md","protocol-freeze.json","request-manifest.jsonl","markets-study.json","mos-study.json","collection-summary.json"]},
         "test_performance_exposed":False}
    for s in ["train","validation"]:
        es=[e for e in accepted if e["split"]==s]
        pre[s+"_scores"]={k:float(np.mean([score((e["r"] if k=="market" else e["b"] if k=="raw" else [1/len(e["r"])]*len(e["r"]) if k=="uniform" else predict(e,config)),e["y"]) for e in es])) if es else None for k in ["market","candidate","raw","uniform"]}
    save("fit-lock.json",pre)
    print(json.dumps({k:v for k,v in pre.items() if k!="training_grid"},indent=2),flush=True)
...
def bootstrap(values,block,seed,reps=10000):
    ...
    if np.any(valid_counts==0):
        return {"lower_one_sided_97_5":None,"upper_one_sided_97_5":None,"block_days":block,"repetitions":reps,
    ...
    return {"lower_one_sided_97_5":float(np.quantile(draws,.025)),"upper_one_sided_97_5":float(np.quantile(draws,.975)),
            "block_days":block,"repetitions":reps,"finite_repetitions":len(draws),"seed":seed}

def evaluate():
    fit=load("fit-lock.json"); es=load("prepared-events.json"); cov=load("coverage.json"); config=fit["config"]
    ...
    review=load("pretest-review.json")
    if review.get("status")!="PASSED" or review.get("analysis_sha256")!=fit["script_sha256"]:
        raise RuntimeError("Independent pretest review missing or wrong script")
    verify_inputs()
    results={"evaluated_utc":dt.datetime.now(dt.timezone.utc).isoformat(),"config":config,"scientific_hurdle":.001,
             "fit_lock_sha256":hashlib.sha256((ROOT/"fit-lock.json").read_bytes()).hexdigest(),"splits":{}}
    per=[]
    for e in es:
        vals={"market":score(e["r"],e["y"]),"candidate":score(predict(e,config),e["y"]),"raw":score(e["b"],e["y"]),"uniform":score([1/len(e["r"])]*len(e["r"]),e["y"])}
        per.append({k:e[k] for k in ["date","event","split","bin_count","raw_mass","max_candle_age","mean_spread","mos_forecast"]}|vals|{"gain":vals["market"]-vals["candidate"]})
    for s in ["train","validation","test_a","test_b","test_pooled"]:
        ...
        if s in ["test_a","test_b","test_pooled"]:
            ...
            seed={"test_a":20260921,"test_b":20260922,"test_pooled":20260923}[s]
    ...
    passes=minimums and all(x["bootstrap_14"]["lower_one_sided_97_5"] is not None and x["bootstrap_14"]["lower_one_sided_97_5"]>.001 for x in [a,b])
    results["minimums_pass"]=minimums; results["replicated_screen_pass"]=bool(passes)
    results["strict_claim_status"]="EXPLORATORY_NUMERICAL_SCREEN_PASSED_TIMING_UNVERIFIED" if passes else "NOT_ESTABLISHED"
    results["historical_publication_timing_verified"]=False
    results["family_wise_error_control_established"]=False
...
def selfcheck():
    r=np.array([.1,.2,.7])
    example={"r":r,"intervals":[[None,89],[90,91],[92,None]],"mos_forecast":90.5}
    assert np.array_equal(predict(example,[0,3,0]),r)
    assert np.isclose(predict(example,[0,3,1]).sum(),1)
    expected=math.erf(1/(3*math.sqrt(2)))
    assert abs(predict(example,[0,3,1])[1]-expected)<1e-12
    ...
    assert abs(same["lower_one_sided_97_5"]-.003)<1e-12
    assert bootstrap([math.nan]*30,14,1,100)["lower_one_sided_97_5"] is None
