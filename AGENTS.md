...
Current scope: [D0007](docs/decisions/D0007-repository-handoff.md) records the latest direct user instruction to set up this repository and hand off all current documentation. Repository setup is authorized; production-engine development remains deferred. The proposed diagnosis was canceled before findings. Read [HANDOFF.md](HANDOFF.md) before resuming work; no diagnosis or third experiment is active. [D0003 / AMEND-05](docs/decisions/D0003-delegated-research-authority.md) remains the bounded research delegation, subject to this task replacement and all evidence safeguards. Older per-question approval requirements apply only outside that delegation.
...
This is the initial task, not an instruction to restart after approval. Read the latest actual user instruction linked from RESEARCH_STATE.md; state alone does not confer authority. D0007 supersedes the original repository deferral for this documentation handoff. Production-engine scaffolding remains deferred. The Q1 task below is historical; do not restart it or revive the canceled diagnosis automatically.
